...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        mixed_query_layer = self.query(hidden_states, self.qact_sa_input.quantizer)
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = self.transpose_for_scores(
                self.key(encoder_hidden_states, self.qact_sa_input.quantizer)
            )  # cross attnetion
            value_layer = self.transpose_for_scores(
                self.value(encoder_hidden_states, self.qact_sa_input.quantizer)
            )
            attention_mask = encoder_attention_mask
        elif past_key_value is not None:
            key_layer = self.transpose_for_scores(
                self.key(hidden_states, self.qact_sa_input.quantizer)
            )  # self attention
            value_layer = self.transpose_for_scores(
                self.value(hidden_states, self.qact_sa_input.quantizer)
            )
        ...
        attention_scores = self.mul_qk(query_layer, key_layer)
        ...
        attention_probs = self.sftm(attention_scores)
        ...
        context_layer = self.mul_sv(attention_probs, value_layer)
...
class BertSelfOutput(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.dense(hidden_states, self.qact_so_input.quantizer)
        hidden_states = self.dropout(hidden_states)
        if self.quant == True:
            hidden_states = self.LayerNorm(
                (hidden_states.bfloat16() + input_tensor.bfloat16()).bfloat16()
            )
        else:
...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.dense(hidden_states, self.qact_itmd_input.quantizer)  # fc1
...
class BertOutput(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.dense(hidden_states, self.qact_fno_input.quantizer)  # fc2
        hidden_states = self.dropout(hidden_states)
        if self.quant == True:
            hidden_states = self.LayerNorm(
                (hidden_states.bfloat16() + input_tensor.bfloat16()).bfloat16()
