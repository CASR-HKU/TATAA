...
class HMQLinear(nn.Linear):
    ...
    def forward(self, x, mp=None, layer_name='', quant_type='int8', node_type='mlp'):
        if self.calibrate:
            self.quantizer.observer.update(self.weight)
            if self.last_calibrate:
                self.quantizer.update_quantization_params(x)
        if not self.quant:
            x_ln = F.linear(x, self.weight, self.bias)
            if mp is not None:
                mp.parse_layer(layer_name, node_type)
                # print("x shape: ", x.shape)
                # print("weight shape: ", self.weight.shape)
                # print("Out shape: ", x_ln.shape)
                mp.parse_ops('matmul', in_data=self.weight.transpose(0,1), in_data_name='', 
                            in_tmp=x.flatten(0,1), in_tmp_name='', 
                            out_data=x_ln.flatten(0,1), out_data_name='', 
    ...
    def forward(self, q, k, layer_name='', mp=None):
        ...
        if not self.quant:
            ...
            if mp is not None:
                ...
                mp.parse_ops('matmul', in_data=k.flatten(0,1), in_data_name='', in_tmp=q.flatten(0,1), in_tmp_name='', out_data=x, out_data_name='', feature_dict={'scale': 0.1, 'quant_type': 'fp16'}, int_ops=True, keep_out=True)
...
class HMQSoftmax(nn.Module):
    ...
    def forward(self, x, layer_name='', mp=None, C_dict=None):
        ...
        # print("sftm mua x: ", x.shape)
        # print("sftm mua out: ", exp_x_sum.shape)
        if mp is not None:
