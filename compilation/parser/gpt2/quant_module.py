...
class HMQConv1D(nn.Conv1d):
    ...
    def forward(self, x, x_quantizer=None, mp=None, layer_name='', quant_type='int8', node_type='mlp'):
        ...
        addmm_x = torch.addmm(self.bias, x.view(-1, x.size(-1)), self.weight)

        if mp is not None:
            mp.parse_layer(layer_name, node_type)
            mp.parse_ops('matmul', in_data=x.view(-1, x.size(-1)), in_data_name='', 
                        in_tmp=self.weight, in_tmp_name='', 
                        out_data=addmm_x, out_data_name='', 
                        feature_dict={'bias_shape': self.bias.shape, 'scale': 0.1, 'quant_type': quant_type}, int_ops=True)
        x = addmm_x.view(size_out)
        return x
...
class HMQMulQK(nn.Module):
    ...
    def forward(self, q, k, layer_name='', mp=None):
        ...
        if mp is not None:
            ...
            mp.parse_ops('matmul', in_data=k_trans, in_data_name='', in_tmp=q, in_tmp_name='', out_data=x, out_data_name='', 
                        feature_dict={'scale': 0.1, 'quant_type': 'fp16'}, int_ops=True, keep_out=True)
